...
# ── OUI fleet table (national, no state filter) ────────────────────────────────
...
SQL_CONCENTRATION = f"""SELECT
  ...
  COUNT(DISTINCT {OUI_EXPR}) AS "Distinct OUIs",
  ROUND(COUNT(DISTINCT {OUI_EXPR})::numeric / NULLIF(COUNT(*), 0), 4) AS "Diversity ratio",
  (SELECT {OUI_EXPR} FROM app.wigle_v2_networks_search v2b
   WHERE v2b.ssid ILIKE '%' || '$ssid_pattern' || '%' AND v2b.country='US' AND v2b.region=v2.region
   GROUP BY {OUI_EXPR} ORDER BY COUNT(*) DESC LIMIT 1) AS "Dominant OUI",
  CASE
    WHEN ROUND(COUNT(DISTINCT {OUI_EXPR})::numeric / NULLIF(COUNT(*),0), 4) < 0.10 THEN 'Fleet signal'
    WHEN ROUND(COUNT(DISTINCT {OUI_EXPR})::numeric / NULLIF(COUNT(*),0), 4) < 0.30 THEN 'Moderate concentration'
    ELSE 'High diversity'
...
classified AS (
  ...
  WHERE NOT EXISTS (SELECT 1 FROM app.wigle_v3_network_details v3 WHERE v3.netid = b.bssid)
    AND {hw_class_case()} IN ('mobile_command','fleet_vehicle','residential_agent','enterprise')
),
tiers AS (
  SELECT
    CASE hw_class
  ...
  FROM classified GROUP BY hw_class
)
SELECT * FROM tiers
UNION ALL
...
**B20073 — ⚠ Unregistered (33 states)**: No IEEE manufacturer registration. National spread with consistent office proximity. The Tacoma pair (`B2:00:73:5F:D5:E5/E6`) is the anchor record. Possible explanations: custom-manufactured device for government use, or recently registered OUI not yet in the public database.
