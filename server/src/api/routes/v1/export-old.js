...
// Middleware to require authentication (disabled for exports)
const requireAuth = (req, res, next) => {
  // Skip auth for all export requests
  return next();
};
...
router.get('/json', requireAuth, async (req, res) => {
  try {
    const [observations, networks] = await Promise.all([
      query('SELECT * FROM public.observations ORDER BY time DESC LIMIT 50000'),
...
router.get('/csv', requireAuth, async (req, res) => {
  try {
    ...
    const headers = ['bssid', 'latitude', 'longitude', 'signal_dbm', 'observed_at', 'radio_type'];
    const csv = [
      headers.join(','),
      ...result.rows.map((row) =>
        headers
