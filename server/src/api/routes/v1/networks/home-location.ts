 ...
 * Manage home/base location for distance calculations
 */
...
/**
 * GET /home-location - Get current home location
 */
router.get('/home-location', async (req, res, next) => {
  try {
    const result = await query(
      `SELECT latitude, longitude, radius, created_at
       FROM app.location_markers WHERE marker_type = 'home'
       ORDER BY created_at DESC LIMIT 1`
    );

    if (result.rows.length === 0) {
      return res.json({ latitude: 43.02345147, longitude: -83.69682688, radius: 100 });
    }
...
/**
 * POST /admin/home-location - Set home location
 */
