...
export class AnalyticsModule {
  ...
  public buildAnalyticsQueries(options: AnalyticsOptions = {}): AnalyticsQueries {
    ...
    const typeExpr = useLatestPerBssid
      ? OBS_TYPE_EXPR('network_set')
      : OBS_TYPE_EXPR('filtered_obs');
    ...
    return {
      ...
      temporalActivity: {
        sql: `${cte} SELECT DATE_TRUNC('hour', time) as period, COUNT(*) as count FROM filtered_obs GROUP BY period ORDER BY period`,
        params: [...params],
      },
      radioTypeOverTime: {
        sql: `${cte} SELECT DATE_TRUNC('day', time) as period, ${OBS_TYPE_EXPR('o')} as type, COUNT(*) as count FROM filtered_obs o GROUP BY period, type ORDER BY period`,
        params: [...params],
      },
      threatTrends: {
        sql: `${baseCtes} 
