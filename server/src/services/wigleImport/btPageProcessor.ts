...
/**
 * Commit a successfully fetched BT/BLE page into wigle_v2_bluetooth_search
 * and update the run progress counters. Mirrors pageProcessor.ts but targets
 * the BT table via importWigleBtSearchResult.
 */
const processSuccessfulBtPage = async (
  runId: number,
  pageNumber: number,
  ...
  try {
    ...
    await client.query(
      `INSERT INTO app.wigle_import_run_pages (
          run_id, page_number, request_cursor, next_cursor, rows_returned, rows_inserted,
          success, error_message, fetched_at, updated_at
    ...
    logger.info('[WiGLE BT Import] Page committed', {
      runId,
      pageNumber,
...
export { processSuccessfulBtPage };
