-- Migration: 20260522_009_deterministic_sibling_sieve_find_sibling_radios.sql
-- Overrides app.find_sibling_radios PL/pgSQL function with vendor-classified deterministic routing array and internal 16-node cluster ceiling constraints.
    ...
    CASE
      ...
      ELSE
        CASE
          ...
          WHEN matched_octets_count >= 5 AND NOT (octets_1_5_identical AND octet_6_delta <= 3) THEN 'Unnamed Recursive (Class B)'
          -- Strict sequential match -> Class C behavior (Consumer/Generic)
          WHEN octets_1_5_identical AND octet_6_delta <= 3 THEN 'Class C'
          ELSE NULL
        END
