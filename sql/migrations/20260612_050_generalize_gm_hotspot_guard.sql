-- Migration: 20260612_050_generalize_gm_hotspot_guard.sql
-- Generalize the GM vehicle hotspot guard to prevent mismatched vehicle SSIDs from pairing,
-- and place it at the top of the CASE expression to avoid sequential rule false-positives.
  ...
  WHERE n.bssid <> UPPER(p_bssid)
    ...
    AND (
      ...
      OR (
        ...
        AND (
          -- Rotating byte 2 (same bytes 3-6)
          SUBSTRING(n.bssid, 7, 11) = (SELECT SUBSTRING(bssid, 7, 11) FROM t)
          -- Or rotating bytes 2-3 (same byte 1 and bytes 4-6)
          OR (
            SUBSTRING(n.bssid, 1, 2) = (SELECT SUBSTRING(bssid, 1, 2) FROM t)
            AND SUBSTRING(n.bssid, 10, 8) = (SELECT SUBSTRING(bssid, 10, 8) FROM t)
          )
        )
      )
    )
...
classified AS (
  SELECT
    ...
    CASE
      ...
      -- Reject if either SSID represents a GM vehicle hotspot and they are not identical.
      -- Evaluated first to prevent mismatched vehicles from matching any subsequent rule.
      WHEN (
          ...
          OR sibling_ssid ILIKE 'BUICK%' OR sibling_ssid ILIKE 'CHEVROLET%' OR sibling_ssid ILIKE 'GMC%' OR sibling_ssid ILIKE 'CADILLAC%'
        )
      ...
      -- GM Vehicle Hotspot XOR Rule:
      -- Octets 1, 4, 5, and 6 are frozen while octets 2 and 3 are bitwise complements.
      WHEN has_la_bit
        AND (SUBSTRING(target_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7') OR SUBSTRING(sibling_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7'))
        AND (('x' || replace(SUBSTRING(target_bssid, 4, 2), ':', ''))::bit(8)::int # (('x' || replace(SUBSTRING(sibling_bssid, 4, 2), ':', ''))::bit(8)::int)) = 255
        AND (('x' || replace(SUBSTRING(target_bssid, 7, 2), ':', ''))::bit(8)::int # (('x' || replace(SUBSTRING(sibling_bssid, 7, 2), ':', ''))::bit(8)::int)) = 255
        AND SUBSTRING(target_bssid, 1, 2) = SUBSTRING(sibling_bssid, 1, 2)
        AND SUBSTRING(target_bssid, 10, 8) = SUBSTRING(sibling_bssid, 10, 8)
        THEN 'GM Vehicle Hotspot XOR (Class A)'

      -- GM Vehicle Hotspot Shift Rule:
      -- Octets 1 and 3-6 are frozen while octet 2 right-shifts by one.
      WHEN has_la_bit
        AND (SUBSTRING(target_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7') OR SUBSTRING(sibling_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7'))
        AND (
          ((('x' || replace(SUBSTRING(target_bssid, 4, 2), ':', ''))::bit(8)::int) >> 1) = (('x' || replace(SUBSTRING(sibling_bssid, 4, 2), ':', ''))::bit(8)::int)
          OR ((('x' || replace(SUBSTRING(sibling_bssid, 4, 2), ':', ''))::bit(8)::int) >> 1) = (('x' || replace(SUBSTRING(target_bssid, 4, 2), ':', ''))::bit(8)::int)
        )
        AND SUBSTRING(target_bssid, 1, 2) = SUBSTRING(sibling_bssid, 1, 2)
        AND SUBSTRING(target_bssid, 7, 11) = SUBSTRING(sibling_bssid, 7, 11)
        THEN 'GM Vehicle Hotspot Shift (Class A)'

      -- GM Vehicle Hotspot Oct4-0x10 Rule:
      -- Octets 1-3 and 5-6 are frozen while octet 4 differs by exactly 16.
      WHEN has_la_bit
        AND (SUBSTRING(target_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7') OR SUBSTRING(sibling_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7'))
        AND SUBSTRING(target_bssid, 1, 8) = SUBSTRING(sibling_bssid, 1, 8)
        AND SUBSTRING(target_bssid, 13, 5) = SUBSTRING(sibling_bssid, 13, 5)
        AND ABS(
          (('x' || replace(SUBSTRING(target_bssid, 10, 2), ':', ''))::bit(8)::int) -
          (('x' || replace(SUBSTRING(sibling_bssid, 10, 2), ':', ''))::bit(8)::int)
        ) = 16
        THEN 'GM Vehicle Hotspot Oct4-0x10 (Class A)'

      -- GM Vehicle Hotspot Oct4-0x60 Rule:
      -- Octets 1-3 and 5-6 are frozen while octet 4 differs by exactly 96.
      WHEN has_la_bit
        AND (SUBSTRING(target_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7') OR SUBSTRING(sibling_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7'))
        AND SUBSTRING(target_bssid, 1, 8) = SUBSTRING(sibling_bssid, 1, 8)
        AND SUBSTRING(target_bssid, 13, 5) = SUBSTRING(sibling_bssid, 13, 5)
        ...
        THEN 'GM Vehicle Hotspot Oct4-0x60 (Class A)'

      -- GM Vehicle Hotspot Oct4-0x20 Rule:
      -- Octets 1-3 and 5-6 are frozen while octet 4 differs by exactly 32.
      WHEN has_la_bit
        AND (SUBSTRING(target_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7') OR SUBSTRING(sibling_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7'))
        AND SUBSTRING(target_bssid, 1, 8) = SUBSTRING(sibling_bssid, 1, 8)
        AND SUBSTRING(target_bssid, 13, 5) = SUBSTRING(sibling_bssid, 13, 5)
        ...
        THEN 'GM Vehicle Hotspot Oct4-0x20 (Class A)'

      -- GM Vehicle Hotspot Sequential Rule:
      -- Octets 1-5 are frozen while the final octet differs by 1, 2, or 3.
      WHEN has_la_bit
        AND (SUBSTRING(target_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7') OR SUBSTRING(sibling_bssid, 1, 8) IN ('02:24:A5', '02:49:A5', '02:92:A5', '00:92:A5', '12:E8:A7'))
        AND SUBSTRING(target_bssid, 1, 14) = SUBSTRING(sibling_bssid, 1, 14)
        AND ABS(
          (('x' || replace(SUBSTRING(target_bssid, 16, 2), ':', ''))::bit(8)::int) -
          (('x' || replace(SUBSTRING(sibling_bssid, 16, 2), ':', ''))::bit(8)::int)
        ) <= 3
        THEN 'GM Vehicle Hotspot Seq (Class A)'
      ...
      -- Mist Systems Cross-Band Sibling Rule:
      -- Groups dual-radio chassis where 2.4 GHz and 5 GHz radios share the same
      -- five-octet base and low-nibble SSID slot, with high nibble separated by 2.
      WHEN (SUBSTRING(target_bssid, 1, 8) IN ('5C:5B:35', 'D4:DC:09', 'D4:20:B0') OR SUBSTRING(sibling_bssid, 1, 8) IN ('5C:5B:35', 'D4:DC:09', 'D4:20:B0'))
        AND SUBSTRING(target_bssid, 1, 14) = SUBSTRING(sibling_bssid, 1, 14)
        AND target_bssid ~ '^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$'
        AND sibling_bssid ~ '^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$'
        AND (('x' || SUBSTRING(target_bssid, 16, 2))::bit(8)::int & 15) = (('x' || SUBSTRING(sibling_bssid, 16, 2))::bit(8)::int & 15)
        AND ABS(
          ((('x' || SUBSTRING(target_bssid, 16, 2))::bit(8)::int) >> 4) -
          ((('x' || SUBSTRING(sibling_bssid, 16, 2))::bit(8)::int) >> 4)
        ) = 2
        AND (
          (frequency_target BETWEEN 2400 AND 2500 AND frequency_sibling BETWEEN 5100 AND 5900)
          OR (frequency_sibling BETWEEN 2400 AND 2500 AND frequency_target BETWEEN 5100 AND 5900)
        )
        THEN 'Mist Systems Cross-Band (Class A)'
      ...
      -- Netgear Dual-Band Sibling Rule:
      -- Groups Netgear gateway virtual APs sharing OUI 6C:CD:D6, same suffix byte 6, and byte 4 delta <= 3.
      WHEN split_part(target_bssid, ':', 1) = '6C' AND split_part(target_bssid, ':', 2) = 'CD' AND split_part(target_bssid, ':', 3) = 'D6'
        AND split_part(sibling_bssid, ':', 1) = '6C' AND split_part(sibling_bssid, ':', 2) = 'CD' AND split_part(sibling_bssid, ':', 3) = 'D6'
        AND split_part(target_bssid, ':', 6) = split_part(sibling_bssid, ':', 6)
        AND octet_4_delta <= 3
        THEN 'Netgear Dual-Band (Class A)'

      -- Arcadyan HOME-EE7D Sibling Rule:
      -- Groups Arcadyan gateway virtual APs where bytes 2-5 match exactly as B0:66:EB:E1,
      -- and the last-octet delta is <= 7.
      WHEN split_part(target_bssid, ':', 2) = 'B0' AND split_part(target_bssid, ':', 3) = '66'
        AND split_part(target_bssid, ':', 4) = 'EB' AND split_part(target_bssid, ':', 5) = 'E1'
        AND split_part(sibling_bssid, ':', 2) = 'B0' AND split_part(sibling_bssid, ':', 3) = '66'
        AND split_part(sibling_bssid, ':', 4) = 'EB' AND split_part(sibling_bssid, ':', 5) = 'E1'
        AND octet_6_delta <= 7
        THEN 'Arcadyan HOME-EE7D (Class A)'

      -- HP Aruba Cross-Band Sibling Rule:
      -- Groups Aruba dual-radio APs where 2.4 GHz and 5 GHz radios share the same
      -- five-octet base and SSID slot index, with high nibble incrementing by band.
      WHEN (SUBSTRING(target_bssid, 1, 8) IN ('60:26:EF', 'E8:10:98') OR SUBSTRING(sibling_bssid, 1, 8) IN ('60:26:EF', 'E8:10:98'))
        AND SUBSTRING(target_bssid, 1, 14) = SUBSTRING(sibling_bssid, 1, 14)
        AND (('x' || split_part(target_bssid, ':', 6))::bit(8)::int & 15) = (('x' || split_part(sibling_bssid, ':', 6))::bit(8)::int & 15)
        AND ABS(
          ((('x' || split_part(target_bssid, ':', 6))::bit(8)::int) >> 4) -
          ((('x' || split_part(sibling_bssid, ':', 6))::bit(8)::int) >> 4)
        ) = 1
        THEN 'HP Aruba Cross-Band (Class A)'
...
SELECT
  ...
  END AS confidence,
  CASE
    WHEN assigned_bucket IN ('Class A', 'Unnamed Recursive (Class A)', 'Ubiquiti UniFi VAP (Class A)', 'Mist Systems VAP (Class A)', 'GM Vehicle Hotspot (Class A)', 'Comcast Vantiva (Class A)', 'Netgear Dual-Band (Class A)', 'Arcadyan HOME-EE7D (Class A)') THEN 'o2-o5'::text
