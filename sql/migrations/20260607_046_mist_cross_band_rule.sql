-- Migration: 20260607_046_mist_cross_band_rule.sql
-- Add Mist Systems cross-band sibling rule for dual-radio APs sharing a five-octet base
-- and final-octet low nibble, with the high nibble differing by exactly 2 between bands.
...
classified AS (
  SELECT
    ...
    CASE
      ...
      -- Groups same-vehicle virtual APs on different bands where the vehicle SSID matches exactly
      -- under one of the three deterministic MAC structures:
      -- 1. Same bytes 1, 3, 4, 5, 6 (rotating byte 2)
      -- 2. Same bytes 1, 4, 5, 6 (rotating bytes 2-3)
      -- 3. Same bytes 1, 2, 3, 5, 6 + same byte 4 low nibble (rotating byte 4)
      WHEN COALESCE(target_ssid, '') = COALESCE(sibling_ssid, '')
        AND (
        ...
        AND (
          -- Structure 1 & 2: Same byte 1 and same bytes 4-6 (covers rotating byte 2 or rotating bytes 2-3)
          (
            split_part(target_bssid, ':', 1) = split_part(sibling_bssid, ':', 1)
          ...
          -- Structure 3: Same bytes 1-3, same bytes 5-6, same byte 4 low nibble
          OR (
            split_part(target_bssid, ':', 1) = split_part(sibling_bssid, ':', 1)
            AND split_part(target_bssid, ':', 2) = split_part(sibling_bssid, ':', 2)
