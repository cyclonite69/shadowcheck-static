-- Migration: 20260504_wigle_networks_mv_add_bluetooth.sql
-- Recreates app.api_wigle_networks_mv to include BT/BLE rows from
-- wigle_v2_bluetooth_search and adds the mfgrid column for all rows.
-- BT rows are appended via UNION ALL; WiFi rows take precedence on BSSID collisions.
...
wifi_bssids AS (
  -- Canonical set of WiFi BSSIDs to exclude from the BT UNION branch
...
-- ── WiFi rows (existing shape + NULL mfgrid) ───────────────────────────────
SELECT
...
-- ── BT/BLE rows (WiFi takes precedence on BSSID collision) ────────────────
SELECT
  bt.bssid,
