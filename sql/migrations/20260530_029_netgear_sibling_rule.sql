-- Migration: 20260530_029_netgear_sibling_rule.sql
-- Add Netgear Dual-Band sibling rule for 6C:CD:D6 BSSID families
-- to automatically resolve virtual APs on different bands where byte 4 delta is <= 3
-- and final octet matches exactly.
...
SELECT
  ...
  END AS confidence,
  CASE
    WHEN assigned_bucket IN ('Class A', 'Unnamed Recursive (Class A)', 'Ubiquiti UniFi VAP (Class A)', 'Mist Systems VAP (Class A)', 'GM Vehicle Hotspot (Class A)', 'Comcast Vantiva (Class A)', 'Netgear Dual-Band (Class A)') THEN 'o2-o5'::text
