-- Add fleet-fleet guard to refresh_network_sibling_pairs.
--
-- The find_sibling_radios function already excludes fleet SSIDs from the
-- probabilistic path, but the refresh function's ON CONFLICT DO UPDATE could
-- still update stale fleet-fleet pairs if they somehow survive in the hits CTE
-- (e.g. via ssid_exact_sequential with a same-4-octet fleet match that later
-- gets re-evaluated). Adding an explicit guard at the INSERT level ensures
-- fleet-fleet pairs (both sides have fleet SSIDs AND first 5 octets differ)
-- can never be written or updated by the refresh job.
...
BEGIN
  ...
  WHERE f.confidence >= p_min_candidate_conf
    ...
    -- Never pair two fleet-SSID devices unless first 5 octets are identical.
    -- Fleet SSIDs (mdt, GreatLakesMobile, etc.) are shared across many physical
    -- units. Two units with the same fleet SSID are only siblings if they share
    -- the same hardware block (octets 1–5 identical, last octet delta 1–3).
    AND NOT (
      lower(regexp_replace(coalesce(f.ssid1, ''), '[^a-z0-9]+', '', 'g')) IN (
        'greatlakesmobile','mdt','xfinitywifi','xfinitymobile',
        'mtasmartbus','kajeetsmartbus','somguest','somiot'
      )
      AND lower(regexp_replace(coalesce(f.ssid2, ''), '[^a-z0-9]+', '', 'g')) IN (
        'greatlakesmobile','mdt','xfinitywifi','xfinitymobile',
        'mtasmartbus','kajeetsmartbus','somguest','somiot'
      )
      AND NOT (
        upper(split_part(f.bssid1, ':', 1)) = upper(split_part(f.bssid2, ':', 1))
        AND upper(split_part(f.bssid1, ':', 2)) = upper(split_part(f.bssid2, ':', 2))
        AND upper(split_part(f.bssid1, ':', 3)) = upper(split_part(f.bssid2, ':', 3))
        AND upper(split_part(f.bssid1, ':', 4)) = upper(split_part(f.bssid2, ':', 4))
        AND upper(split_part(f.bssid1, ':', 5)) = upper(split_part(f.bssid2, ':', 5))
      )
    )
