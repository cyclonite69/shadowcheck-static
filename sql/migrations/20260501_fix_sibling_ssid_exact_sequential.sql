-- Migration: add ssid_exact_sequential deterministic rule to find_sibling_radios
--
-- BUG: Networks with identical SSIDs and last octet delta ≤ 2 were scoring 0.85
-- via the probabilistic ssid_exact path and falling below the 0.90 insertion
-- threshold when frequency data was absent.
--
-- FIX: Add a new deterministic CTE (ssid_exact_sequential) that fires before any
-- penalty logic. Match criteria: SSIDs case-insensitively equal (non-null/non-empty)
-- + last octet delta 1–2. No MAC prefix requirement. Confidence 1.000, no penalties.
--
-- This mirrors the existing last_octet_sequential rule but is SSID-anchored rather
-- than requiring octets 1–5 to match, covering cross-OUI same-device pairs.
...
-- Canonical multi-radio device pattern (shared MAC block, sequential last octet).
...
-- Deterministic rule 2: identical SSID (case-insensitive, non-null/non-empty),
-- last octet delta 1–2, any MAC prefix. Covers cross-OUI same-device pairs that
-- the probabilistic ssid_exact path misses when frequency data is absent.
-- Excludes pairs already captured by sequential_siblings to avoid duplicates.
ssid_exact_sequential AS (
  ...
  JOIN app.networks n
    ...
    -- Exclude pairs already captured by sequential_siblings (octets 1–5 identical,
    -- delta 1–3 already covers delta 1–2 when o5 matches).
    AND NOT (
...
-- Excludes pairs already captured by either deterministic rule.
c AS (
  SELECT
    t.bssid AS target_bssid,
    n.bssid AS sibling_bssid,
