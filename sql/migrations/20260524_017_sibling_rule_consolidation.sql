-- Migration: 20260524_017_sibling_rule_consolidation.sql
-- Finalized sibling detection rule set: AirLink/Sierra delta-1 twins plus the
-- Mist Systems same-SSID same-band guardrail.
...
classified AS (
  SELECT
    ...
    CASE
      ...
      WHEN matched_octets_count >= 5 AND octet_6_delta <= 7 THEN
...
cluster_counts AS (
  ...
  WHERE assigned_bucket IS NOT NULL
    AND NOT (
      oui_target IN ('70:90:41', '7C:B6:8D', 'A8:3A:79', 'A8:53:7D', 'A8:F7:D9', 'AC:23:16', 'C8:78:67', 'D4:20:B0', 'D4:DC:09')
      AND oui_sibling IN ('70:90:41', '7C:B6:8D', 'A8:3A:79', 'A8:53:7D', 'A8:F7:D9', 'AC:23:16', 'C8:78:67', 'D4:20:B0', 'D4:DC:09')
