-- Migration: 20260528_020_harden_cradlepoint_class_a_delta3.sql
-- Harden Cradlepoint (OUI 00:30:44) sibling matching with two interlocking changes:
--
-- 1. Class A gate: require octets_2_5_identical AND octet_6_delta <= 3.
--    The prior rule had no delta gate, yielding avg delta=59.49 across 443 pairs
--    and confirmed cross-vehicle fleet cliques in MTA/Kajeet SmartBus data.
--
-- 2. Class B exclusion: add AND NOT is_known_cradlepoint to the Class B WHEN.
--    Without this, Cradlepoint pairs that fail the Class A delta gate (delta>3)
--    fall through to Unnamed Recursive (Class B) via the matched_octets>=5 path.
--    The existing vendor guard (NOT octets_1_5_identical) does not block same-OUI
--    same-first-5-octet Cradlepoint pairs because octets_1_5_identical=TRUE for them.
--    The fix: Cradlepoint must only qualify via Class A or produce NULL.
--
-- Verified safe:
--   keep  00:30:44:CA:44:19 <-> 00:30:44:CA:44:1A  (delta 1, Class A)
--   keep  00:30:44:CA:44:28 <-> 00:30:44:CA:44:29  (delta 1, Class A)
--   reject 00:30:44:CA:44:19/1A <-> CA:44:28/29     (delta 14-16, 0 rows)
--   keep  00:30:44:61:8A:8F <-> 00:30:44:61:8A:90  (delta 1, Class A)
--   keep  00:30:44:1C:CA:9E <-> 00:30:44:1C:CA:A0  (delta 2, Class A)
--   reject 00:30:44:A2:54:CE <-> 00:30:44:A2:54:D3  (delta 5, 0 rows — no Class A, no Class B)
--
-- Persisted pairs: the 357 existing Class A pairs with d_last_octet > 3 remain in
-- app.network_sibling_pairs until a targeted recompute/prune pass is explicitly run.
-- This migration only updates the forward detection function.
...
classified AS (
  SELECT
    ...
    octet_4_delta,
    CASE
      -- Cradlepoint (OUI 00:30:44): requires octets_2_5_identical AND delta <= 3.
      -- The delta <= 3 gate prevents cross-vehicle false cliques in fleet deployments
      -- (MTA SmartBus, Kajeet) where sequential MAC allocation across buses
      -- creates pairs with high deltas that look positionally similar.
