-- Dry-run Validation Script: 20260530_verify_arcadyan_sibling_rule_dryrun.sql
--
-- Dry-runs and validates the addition of the Arcadyan HOME-EE7D Sibling Rule.
--
-- Scope:
-- 1. Evaluates all newly matched Arcadyan gateway virtual AP pairs.
...
CREATE TEMP TABLE temp_new_arcadyan_siblings AS
WITH seed_networks AS (
  SELECT bssid, ssid
  FROM app.networks
  WHERE bssid LIKE '%:B0:66:EB:E1:%' OR bssid LIKE '%B0:66:EB:E1%'
)
SELECT DISTINCT
...
WHERE f.rule = 'Arcadyan HOME-EE7D (Class A)';

-- Log count of newly detected sibling pairs
SELECT COUNT(*) AS total_detected_arcadyan_siblings FROM temp_new_arcadyan_siblings;
...
JOIN temp_new_arcadyan_siblings t
