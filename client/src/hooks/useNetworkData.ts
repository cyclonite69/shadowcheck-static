...
import { mapApiRowToNetwork } from './network/networkMappers';
import { serializeFiltersToParams } from './network/networkFilterSerializer';
    ...
    const fetchNetworks = async () => {
      ...
      try {
        ...
        const params = serializeFiltersToParams(
          ...
          enabled,
          sortKeys.join(','),
          sort.map((entry) => entry.direction.toUpperCase()).join(','),
          NETWORK_PAGE_LIMIT,
          pagination.offset,
          locationMode,
          planCheck
