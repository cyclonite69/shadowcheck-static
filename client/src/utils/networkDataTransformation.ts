...
export const mapApiRowToNetwork = (row: any, idx: number): NetworkRow => {
  ...
  return {
    ...
    threat_ml_weight: parseNumericField(row.ml_weight),
    threat_ml_boost: parseNumericField(row.ml_boost),
    threatReasons: [],
    threatEvidence: [],
    stationaryConfidence: parseNumericField(row.stationary_confidence),
    manufacturer: row.manufacturer || null,
    geocoded_address: row.geocoded_address || null,
    geocoded_city: row.geocoded_city || null,
    geocoded_state: row.geocoded_state || null,
    geocoded_postal_code: row.geocoded_postal_code || null,
    geocoded_country: row.geocoded_country || null,
    geocoded_poi_name: row.geocoded_poi_name || null,
    geocoded_poi_category: row.geocoded_poi_category || null,
    geocoded_feature_type: row.geocoded_feature_type || null,
    geocoded_provider: row.geocoded_provider || null,
    geocoded_confidence: parseNumericField(row.geocoded_confidence),
