...
function fieldRow(label: string, value: string, title?: string, allowWrap?: boolean): string {
  ...
  return `<div ${titleAttr} style="display:grid;grid-template-columns:130px 1fr;align-items:${allowWrap ? 'flex-start' : 'center'};min-height:26px;padding:3px 12px;border-bottom:1px solid rgba(255,255,255,0.05);">
    <div style="font-size:9px;text-transform:uppercase;letter-spacing:0.07em;color:rgba(255,255,255,0.38);white-space:nowrap;padding-top:${allowWrap ? '3px' : '0px'};">${label}</div>
...
export const renderNetworkTooltip = (props: any): any => {
  ...
  const hasPOIName =
    !isMissingValue(props.geocoded_poi_name) && String(props.geocoded_poi_name).trim() !== '';
  const hasGeocoded =
    !isMissingValue(props.geocoded_address) && String(props.geocoded_address).trim() !== '';
  const displayAddress = hasPOIName
    ? props.geocoded_poi_name
    : hasGeocoded
...
<div style="width:288px;max-width:min(340px, 90vw);max-height:min(600px, 90vh);background:#1a1d23;border:2px solid ${bc};border-radius:10px;box-shadow:0 8px 32px rgba(0,0,0,0.6);font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;color:#fff;box-sizing:border-box;">
  ...
  ${
    ...
    ${Boolean(props.wigle_match) ? `<div style="display:inline-flex;align-items:center;gap:4px;padding:2px 6px;border-radius:999px;background:rgba(34,197,94,0.12);border:1px solid rgba(34,197,94,0.28);color:#86efac;font-size:10px;font-weight:600;white-space:nowrap;">WiGLE Correlated${Number(props.local_observation_count) > 0 ? ` &middot; ${Number(props.local_observation_count)} obs` : ''}</div>` : `<div style="display:inline-flex;align-items:center;gap:4px;padding:2px 6px;border-radius:999px;background:rgba(251,146,60,0.12);border:1px solid rgba(251,146,60,0.28);color:#fdba74;font-size:10px;font-weight:600;white-space:nowrap;">WiGLE External</div>`}
  </div>`
      : ''
  }
  ...
  ${
    ...
    <div style="padding:6px 12px 8px;background:rgba(255,255,255,0.02);">
      ${!isMissingValue(props.number) && obsTotal > 0 ? `<div style="font-size:10px;color:rgba(255,255,255,0.5);margin-bottom:6px;">OBS #${normalizeDisplay(props.number)} of ${obsTotal}</div>` : ''}
