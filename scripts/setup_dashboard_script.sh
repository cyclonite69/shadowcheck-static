...
class DashboardService {
  ...
  async getMetrics() {
    try {
      const networks = await this.networkRepository.getAllNetworks();

      // Calculate threat levels
      const threatsCritical = networks.filter(n => n.threatScore >= 80).length;
      const threatsHigh = networks.filter(n => n.threatScore >= 60 && n.threatScore < 80).length;
      const threatsMedium = networks.filter(n => n.threatScore >= 40 && n.threatScore < 60).length;
      const threatsLow = networks.filter(n => n.threatScore < 40).length;

      // Count by radio type
      const wifiCount = networks.filter(n => n.type === 'W').length;
      const bleCount = networks.filter(n => n.type === 'E').length;
      const bluetoothCount = networks.filter(n => n.type === 'B').length;
      const lteCount = networks.filter(n => n.type === 'L').length;

      // Active surveillance (networks observed 100+ times)
      const activeSurveillance = networks.filter(n => n.observations >= 100).length;

      // Enriched networks (have geolocation data)
      const enrichedCount = networks.filter(n => n.latitude && n.longitude).length;
  ...
  async getThreats() {
    try {
      ...
      return networks
        .filter(n => n.threatScore > 0 || n.threatLevel)
  ...
  async getNetworkDistribution() {
    try {
      ...
      return {
        wifi: networks.filter(n => n.type === 'W').length,
        ble: networks.filter(n => n.type === 'E').length,
        bluetooth: networks.filter(n => n.type === 'B').length,
        lte: networks.filter(n => n.type === 'L').length,
...
router.get('/v1/dashboard/metrics', async (req, res) => {
  try {
    if (!dashboardService) {
...
router.get('/v1/dashboard/threats', async (req, res) => {
  try {
    if (!dashboardService) {
      return res.status(500).json({ error: 'Dashboard service not initialized' });
    }
...
echo "    app.use('/api', dashboardRoutes.router);"
echo ""
...
echo "    curl http://localhost:3001/api/v1/dashboard/threats"
echo "    curl http://localhost:3001/api/v1/dashboard/summary"
echo ""