...
async function main(): Promise<void> {
  ...
  console.log(
    `📡 APIs available: Overpass, Nominatim${apiKeys.locationiq ? ', LocationIQ' : ''}${apiKeys.opencage ? ', OpenCage' : ''}`
  );
  ...
  for (let i = 0; i < result.rows.length; i += CONCURRENT) {
    ...
    if ((i + CONCURRENT) % 50 === 0 || i + CONCURRENT >= result.rows.length) {
      console.log(
        `  ✓ ${Math.min(i + CONCURRENT, result.rows.length)}/${result.rows.length} (${enriched} enriched, ${((enriched / (i + CONCURRENT)) * 100).toFixed(1)}% success)`
      );
    }
