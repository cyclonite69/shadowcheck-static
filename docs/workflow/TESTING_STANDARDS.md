...
`docs/metrics/coverage_report.md` is generated by `scripts/generate-coverage-report.js`
from `coverage/coverage-summary.json`. It contains:

- **Global Totals** — per-metric covered/total counts and pass/fail against the 60% threshold.
- **Coverage Trend** — delta table comparing the current run against the previous snapshot
  stored in `docs/metrics/coverage_snapshot.json`. Shows Δ covered/total and Δ % for each
  metric. On the first run after the snapshot is absent, this section shows a placeholder.
- **Layer Breakdown** — aggregate and per-file tables for each code group (`etl/`, `scripts/`,
  `server/src/api/routes/`, `server/src/services/`, `server/src/repositories/`,
  `server/src/ (other)`).
- **Next Opportunities** — top 15 files ranked by engineering value, not raw line count.
  Scoring weights: uncovered branches × 3 + uncovered functions × 2 + uncovered statements × 1,
  multiplied by a tier factor (routes/services/validators = 4, repositories = 3, etl/scripts = 2).
  Excludes `.d.ts` files, barrel `index.ts/js` files, and the report generator itself.

The snapshot file (`docs/metrics/coverage_snapshot.json`) is updated on every
`coverage:report` or `coverage:refresh` run and should be committed alongside the report.
