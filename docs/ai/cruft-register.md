...
### Photo layer (`usePhotoLocationLayers`) — only plots `VISINT_UNMATCHED`
...
| **Status**        | `WATCH`                                                                                                                                                                                                |
| **Why it exists** | Built for the VISINT unmatched sentinel workflow. Only queries `WHERE bssid = 'VISINT_UNMATCHED'`.                                                                                                     |
| **Gap**           | Matched media (real BSSIDs with `exif_lat`/`exif_lon` or MV `lat`/`lon`) is not plotted at all. The photo layer icon on the geospatial page is therefore misleading — it only shows the fallback case. |
