...
const CardLibrary = {
    ...
    networkList: {
        ...
        loadData: async function(containerId, columns, filters) {
            try {
                ...
                if (filters.security) networks = networks.filter(n => (n.security || '').includes(filters.security));
    ...
    threatList: {
        ...
        formatCell: function(column, threat) {
            ...
            if (column === 'detected') return threat.detected_at ? new Date(threat.detected_at).toLocaleString() : '-';
    ...
    mapViewer: {
        ...
        initMap: function(containerId) {
            ...
            mapDiv.innerHTML = '<div class="text-center text-secondary" style="padding: 40px;">Map loading...</div>';
        },
        refresh: function(containerId) {
