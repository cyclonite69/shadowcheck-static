...
describe('networkTagsAdminService', () => {
  ...
  describe('upsertNetworkTag', () => {
    it('should update existing tag when bssid exists', async () => {
      ...
      expect(adminQuery).toHaveBeenCalledTimes(4); // check + 3 updates
    });
    ...
    it('should insert new tag when bssid does not exist', async () => {
      ...
      expect(adminQuery).toHaveBeenCalledTimes(4); // check + 3 inserts
    });
    ...
    it('should only perform actions for non-null parameters', async () => {
      ...
      expect(adminQuery).toHaveBeenCalledTimes(2); // check + 1 update (threatTag)
    });
  });
  ...
  describe('individual update/insert functions', () => {
    ...
    it('updateNetworkTagIgnore should call UPDATE', async () => {
      ...
      expect(adminQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE app.network_tags SET is_ignored = $1'), [true, bssid]);
    });

    it('insertNetworkTagIgnore should call INSERT', async () => {
      (adminQuery as jest.Mock).mockResolvedValueOnce({ rows: [mockRow] });
      await networkTagsAdminService.insertNetworkTagIgnore(bssid, true);
      expect(adminQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO app.network_tags (bssid, is_ignored)'), [bssid, true]);
    });

    it('updateNetworkThreatTag should call UPDATE', async () => {
      (adminQuery as jest.Mock).mockResolvedValueOnce({ rows: [mockRow] });
      await networkTagsAdminService.updateNetworkThreatTag(bssid, 'threat');
      expect(adminQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE app.network_tags SET threat_tag = $1'), ['threat', bssid]);
    });

    it('insertNetworkThreatTag should call INSERT', async () => {
      (adminQuery as jest.Mock).mockResolvedValueOnce({ rows: [mockRow] });
      await networkTagsAdminService.insertNetworkThreatTag(bssid, 'threat');
      expect(adminQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO app.network_tags (bssid, threat_tag)'), [bssid, 'threat']);
    });

    it('updateNetworkTagNotes should call UPDATE', async () => {
      (adminQuery as jest.Mock).mockResolvedValueOnce({ rows: [mockRow] });
      await networkTagsAdminService.updateNetworkTagNotes(bssid, 'notes');
      expect(adminQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE app.network_tags SET notes = $1'), ['notes', bssid]);
    });

    it('insertNetworkTagNotes should call INSERT', async () => {
      (adminQuery as jest.Mock).mockResolvedValueOnce({ rows: [mockRow] });
      await networkTagsAdminService.insertNetworkTagNotes(bssid, 'notes');
      expect(adminQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO app.network_tags (bssid, notes)'), [bssid, 'notes']);
    });
  });
  ...
  describe('deleteNetworkTag', () => {
    it('should delete tag and return rowCount', async () => {
      ...
      expect(adminQuery).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM app.network_tags WHERE bssid = $1'), [bssid]);
    });
  });
});
