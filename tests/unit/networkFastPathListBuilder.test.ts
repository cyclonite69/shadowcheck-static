...
jest.mock('../../server/src/services/filterQueryBuilder/modules/networkFastPathPredicates', () => {
  const original = jest.requireActual(
    '../../server/src/services/filterQueryBuilder/modules/networkFastPathPredicates'
  );
  return {
    ...original,
...
describe('networkFastPathListBuilder', () => {
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockReturnValue([{ type: 'test', field: 'field1', value: 'val' }]);
  ...
  it('builds query with default options', () => {
    ...
    expect(result.sql).toContain('ne.has_siblings');
    expect(result.sql).toContain('ne.sibling_count');
    expect(result.sql).toContain('ne.sibling_max_confidence');
    expect(result.sql).toContain('ne.has_strong_sibling');
    expect(result.sql).toContain('ne.sibling_bssids');
  ...
  it('builds query with custom options and replacements in orderBy', () => {
    const options = {
      ...
      offset: 50,
      orderBy:
        'l.observed_at ASC, l.level DESC, l.lat, l.lon, l.accuracy, r.observation_count, r.first_observed_at, r.last_observed_at, ne.stationary_confidence, s.stationary_confidence',
    ...
    expect(result.sql).toContain(
      'ORDER BY ne.observed_at ASC, ne.signal DESC, ne.lat, ne.lon, ne.accuracy_meters, ne.observations, ne.first_seen, ne.last_seen, ne.stationary_confidence, ne.stationary_confidence'
    );
