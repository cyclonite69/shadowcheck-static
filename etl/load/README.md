...
Can also load training facilities from the curated CSV when available.

```bash
# Load all offices (keeps existing non-FBI rows)
tsx etl/load/fbi-locations.ts

# Refresh FBI rows only
tsx etl/load/fbi-locations.ts --refresh

# Limit number of field offices (for testing)
tsx etl/load/fbi-locations.ts --limit=5
```

Legacy FBI supplemental CSV enrichers were removed during ETL consolidation. The active FBI loader is `fbi-locations.ts`.
