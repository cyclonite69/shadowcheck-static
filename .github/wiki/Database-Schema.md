...
**Docs version (repo):** [Network Tables Schema](../../docs/schema/network-tables.md) | [Observation Sources Schema](../../docs/schema/observations-sources.md) | [Database Radio Architecture](../../docs/DATABASE_RADIO_ARCHITECTURE.md)

> **High-level system guide and mapping of the database structure.**
> For exact field definitions, constraints, indexes, and current schema migration commands, always consult the versioned repository schema documentation linked above.

### Core Operational Subsystems

The database comprises several distinct tables and views that manage threat forensic data:

1. **Sibling Grouping & Resolution:**
   - `app.network_sibling_pairs` — Automatically detected BSSID pairings based on BSSID hex patterns and vendor characteristics.
   - `app.network_sibling_overrides` — Manual analyst overrides (allow/block sibling relationships).
   - `app.network_siblings_effective` — A unified, high-performance view resolving automatic pairs and active overrides.

2. **Surveillance & Threat Detection:**
   - `app.surveillance_detections` — Detected surveillance devices generated by rule-based and ML analysis engines.
   - `app.oui_device_groups` — Device manufacturer mappings grouping surveillance-related OUIs.
